...
## Steps to add a new check in the kube-linter project
...
#### **STEP 7:** Build the kube-linter binary from the local project source code. It'll contain the new check this time.
...
- Finally, execute the following command to run all the kube-linter checks on the test `deployment.yaml` manifest
